...
# 【将链接替换为对应剧场门票链接】
browser.get('https://shop.48.cn/tickets/item/4140')  # 跳转页面
...
determine = browser.find_element_by_xpath('//em[@id="seattype2"]')  # 勾选座位类型
browser.execute_script('arguments[0].click();', determine)

while True:  # 无限循环
    if click_time > 0:  # 直到点击一次购票
        break
    if time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()) >= '2021-07-13 20:14:00':  # 设置抢票时间
